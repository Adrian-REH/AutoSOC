...
from .services import process_webdriver_alert_and_notify, rabbitmq_send_ip
import requests
...
def allow_ip(request):
	ip = request.data.get('ip')
	try:
		rabbitmq_send_ip(ip, False)
	except Exception as e:
		print(e)
	return Response({"message": f"Allowed IP: {ip}"})
